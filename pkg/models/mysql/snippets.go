...
func (m *SnippetModel) Get(id int) (*models.Snippet, error) {
	...
	// SQL statement, passing in the untrusted id variables as the value for the
	...
	// to the place you want to copy the data into, and the number of arguments
	// must be exactly the same as the number of columns returned by your statements.
	err := row.Scan(&s.ID, &s.Title, &s.Content, &s.Created, &s.Expires)

	if err != nil {
		// If the query returns no rows, then row.Scan() will return a
		// sql.ErrNoRows error. We use the errors.Is() function check for
		// that error specifically, and return our own models.ErrNoRecord instead.
...
func (m *SnippetModel) Latest() ([]*models.Snippet, error) {
	...
	// error that was encountered during the iteration. It's important to call this
	// - don't assume that a successful iteration was completed over the whole resultset.
	if err = rows.Err(); err != nil {
		return nil, err
	}

	// If everything OK then return the Snippets slice.
	return snippets, nil
}